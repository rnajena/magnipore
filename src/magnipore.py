...
def magnipore(mapping : dict, unaligned : dict, seqs_ids : tuple, alignment_sequences : tuple, alignment_path : str, red_first_sample : dict, red_sec_sample : dict, first_sample_label : str, sec_sample_label : str, working_dir : str, strict : bool, r2 : bool):
    ...
    all.write(f'ref_1\tpos_1\tbase_1\tmotif_1\tsignal_mean_1\tsignal_std_1\tn_datapoints_1\tn_segments_1\tcontained_segments_1\tn_reads_1\t')
    all.write(f'ref_2\tpos_2\tbase_2\tmotif_2\tsignal_mean_2\tsignal_std_2\tn_datapoints_2\tn_segments_2\tcontained_segments_2\tn_reads_2\n')
    ...
    fasta = itertools.chain(
        [SeqIO.SeqRecord(Seq.Seq(''.join(seq)), id ='magnipore_marked_' + str((first_sample_label, sec_sample_label)[i]), name ='magnipore_' + str((first_sample_label, sec_sample_label)[i]), description='X=significant signal change, N=not significant') for i, seq in enumerate(alignment_sequences)], SeqIO.parse(open(alignment_path), 'fasta'))
